...
function viewSales(){
 subtab='orders';
 return `<div class="page-head sales-page-head"><div><h2>Sales</h2><p>Sales Order → order-scoped Makeup → line-specific Shape / Muntin. No global configuration library.</p></div><span class="pill info">Stage 3B</span></div>${salesOrdersPane()}`;
}
function salesOrdersPane(){return soEdit!==null?salesOrderEditor():salesOrderList();}
function salesOrderList(){
 const all=salesListVisible(),rows=all.filter(o=>!salesStatusFilter||salesStatusFilter===salesKindOf(o)+':'+salesListStatus(o)).slice().sort((a,b)=>String(b.createdAt||'').localeCompare(String(a.createdAt||''))),both=salesShow.orders&&salesShow.quotes;
 ...
 const numberOf=id=>((DB.salesOrder||[]).find(x=>x.id===id)||{}).businessNumber||'';
 return `<div class="card sales-list-card"><div class="sales-toolbar"><div class="sales-show"><span>Show</span>${toggle('orders','Orders')}${toggle('quotes','Quotes')}</div><div class="sales-search"><input id="salesOrderSearch" value="${esc(soSearch)}" placeholder="Search number, customer, PO, status…" oninput="salesOrderSearchChange(this)"></div><span class="sales-toolbar-sp"></span><button onclick="salesOrderNew('quote')">+ New Quote</button><button class="pri" onclick="salesOrderNew('order')">+ New Sales Order</button></div>${salesStatusChips(all)}<div class="sales-table-wrap"><table><thead><tr>${both?'<th>Type</th>':''}<th>Number</th><th>Customer</th><th>PO</th><th>Due</th><th>Lines</th><th>Status</th><th class="n">Total</th><th class="n">Receipts</th><th class="n">Balance</th><th></th></tr></thead><tbody>${rows.map(o=>{const q=salesIsQuote(o),from=o.fromQuoteId?numberOf(o.fromQuoteId):'';return `<tr data-order-row="${esc(o.id)}">${both?`<td><span class="pill ${q?'kind-quote':'kind-order'}">${q?'Quote':'Order'}</span></td>`:''}<td><b class="mono">${raw((q?salesQuoteBaseNumber(o):o.businessNumber)||'Auto Number')}</b>${from?` <span class="mut small">from ${raw(from)}</span>`:''}</td><td><b>${raw(salesCustomerDisplay(o.customerId)||'—')}</b></td><td>${raw(o.customerPo||'—')}</td><td>${esc(o.dueDate||'—')}</td><td>${o.lines.length}</td><td>${q?salesQuoteGroupPill(o):salesStatusPill(o)}</td>${salesListBalanceCells(o)}<td class="sales-row-actions"><button class="sm" onclick="salesOrderEdit('${esc(o.id)}')">Open</button><button class="sm dl" onclick="salesOrderDelete('${esc(o.id)}')">×</button></td></tr>`;}).join('')||`<tr><td colspan="${both?11:10}" class="empty">${all.length?'Nothing in this status.':'No Sales Orders yet'}</td></tr>`}</tbody></table></div></div>`;
}
function salesCustomerOptions(){const rows=(DB.customer||[]).filter(c=>c.status!=='archived');return `<option value="">— select Customer —</option>`+rows.map(c=>`<option data-raw value="${esc(c.id)}" ${soDraft.customerId===c.id?'selected':''}>${esc(c.code||'')} · ${esc(c.displayName||c.legalName)}</option>`).join('');}
function salesOrderEditor(){
 const o=soDraft,c=salesFindCustomer(o.customerId),title=salesOrderTitle(o),ro=salesOrderReadOnly(o),q=salesIsQuote(o),fromQuote=o.fromQuoteId?((DB.salesOrder||[]).find(x=>x.id===o.fromQuoteId)||{}).businessNumber:'',customerFlags=c?`${c.onHold?'<span class="pill bad">On Hold</span>':''}${c.poRequired?'<span class="pill warn">PO Required</span>':''}${c.isProspect?'<span class="pill info">Prospect</span>':''}`:'';
 return `<div class="sales-order-editor"><div class="sales-editor-top"><div><div class="sales-order-title"><h3>${esc(title)}</h3>${salesStatusPill(q?salesQuoteShown(o):o)}${customerFlags}</div><div class="mut mono">${esc(q?salesQuoteShown(o).id:o.id)}${fromQuote?' · from quote '+raw(fromQuote):''}</div></div>${q?'':salesPaymentStrip(o)}<div class="sales-editor-actions">${salesHeaderActions(o)}</div></div>
 ${salesStatusStepper(o)}${salesLockBar(o)}${ro?'<div class="sales-readonly" inert>':''}
 <div class="sales-header-compact"><div><label>Customer *</label><select onchange="salesApplyCustomerDefaults(this.value);render()">${salesCustomerOptions()}</select></div><div><label>Customer PO</label><input value="${esc(o.customerPo)}" oninput="soDraft.customerPo=this.value"></div><div><label>Due Date</label><input type="date" value="${esc(o.dueDate)}" onchange="soDraft.dueDate=this.value"></div><div><label>Priority</label><select onchange="soDraft.priority=this.value"><option value="normal" ${o.priority==='normal'?'selected':''}>Normal</option><option value="rush" ${o.priority==='rush'?'selected':''}>Rush</option><option value="critical" ${o.priority==='critical'?'selected':''}>Critical</option></select></div><div><label>Delivery</label><select onchange="soDraft.delivery=this.value"><option value="pickup" ${o.delivery==='pickup'?'selected':''}>Pickup</option><option value="delivery" ${o.delivery==='delivery'?'selected':''}>Delivery</option></select></div><div><label>Currency</label><select onchange="soDraft.currency=this.value;render()"><option ${o.currency==='CAD'?'selected':''}>CAD</option><option ${o.currency==='USD'?'selected':''}>USD</option></select></div><div><label>Branch</label><input value="${esc(o.branch)}" oninput="soDraft.branch=this.value"></div>${q?`<div><label>Valid until</label><input type="date" data-quote-valid value="${esc(o.validUntil)}" title="Empty: the send date plus ${salesQuoteValidDays()} days" onchange="soDraft.validUntil=this.value;render()"></div>`:''}</div>
...
function salesLineCommercialAdjustmentsHtml(line,currency){
 ...
 return `<section class="sales-commercial-adjustments"><div class="sales-commercial-head"><div><b>Commercial adjustments</b><span>Already included once in Unit Price</span></div></div><div class="sales-commercial-table"><div class="sales-commercial-row head"><span>Rule</span><span>Calculation per unit</span><span>Qty</span><span>Line total</span></div>${rows.map(a=>`<div class="sales-commercial-row${a.complete?'':' incomplete'}"><span><b>${esc(a.label)}</b><small>${a.key==='triple'?'Triple IGU':'Billable Area above the limit'}</small></span><span>${a.complete?`${a.base.toFixed(2)} × ${a.percent}% = <b>${a.unitAmount.toFixed(2)} ${esc(currency)}</b>`:'Price incomplete'}</span><span>${a.qty}</span><strong>${a.complete?a.lineAmount.toFixed(2)+' '+esc(currency):'—'}</strong></div>`).join('')}</div></section>`;
}
function salesOrderCommercialAdjustmentsHtml(currency){
 ...
 return `<section class="sales-commercial-adjustments order"><div class="sales-commercial-head"><div><b>Commercial adjustments</b><span>Applied automatically from Makeup type and Billable Area; already included in Unit / Line Price</span></div></div><div class="sales-commercial-table"><div class="sales-commercial-row head"><span>Rule</span><span>Lines / units</span><span>Rate</span><span>Order total</span></div>${groups.map(g=>`<div class="sales-commercial-row${g.incomplete?' incomplete':''}"><span><b>${esc(g.label)}</b><small>${g.key==='triple'?'Triple IGU':'Billable Area above '+salesMetricRules(soDraft).largeThresholdFt2+' ft²'}</small></span><span>${g.lines} / ${g.qty}</span><span>+${g.percent}% of the base unit price</span><strong>${g.incomplete?'—':g.total.toFixed(2)+' '+esc(currency)}</strong></div>`).join('')}</div></section>`;
}
function salesLineServicesModal(line,index){
 ...
 return `<div class="sales-service-modal-back" onclick="if(event.target===this)salesCloseServices()"><div class="sales-service-modal line-modal"><div class="sales-service-modal-head"><div><span><span>Line</span> ${index+1}</span><h3>Services / Processing</h3><small>${s?raw(s.name):'Production Shape'}${line.mark?' · '+raw(line.mark):''}</small></div><button onclick="salesCloseServices()">×</button></div>${rows.length||salesLineCommercialAdjustments(line,soDraft).length?`<div class="sales-service-context"><span>Makeup thickness <b>${esc(thickness)}</b></span><span>Line Qty <b>${esc(line.qty)}</b></span><span>Geometry / quantity <b>read-only</b></span></div>${summary.unpriced?`<div class="sales-pricing-warning"><b>Price not set</b><span><b data-raw>${summary.unpriced}</b> <span>charges have no effective rate. They are not included in the monetary total until a rate is entered.</span></span></div>`:''}${rows.length?`<div class="sales-line-service-table"><div class="sales-line-service-row head"><span>Charge</span><span>Calculated basis</span><span>Catalog</span><span>Order-wide</span><span>Line rate</span><span>Final total</span><span></span></div>${rows.map(function(row){const st=salesChargePricingState(line,row),q=salesPositiveInt(line.qty,1),final=st.effectiveRate==null?null:row.basis*q*st.effectiveRate,lineValue=st.lineRate==null?'':String(st.lineRate),inherited=st.orderRate!=null?st.orderRate:st.catalogRate,origin=st.origin==='line'?'LINE':(st.origin==='order'?'ORDER':(st.origin==='catalog'?'CATALOG':'NO RATE'));return `<div class="sales-line-service-row${st.missing?' unpriced':''}"><span class="sales-price-charge"><b>${esc(row.label)}</b><small>${esc(row.source)}</small></span><span class="sales-price-basis">${esc(salesChargeBasisText(row,line))}</span><span>${st.catalogRate==null?'<span class="sales-price-missing">Not set</span>':esc(salesRateText(st.catalogRate,row.unit,currency))}</span><span>${esc(salesRateText(st.orderRate,row.unit,currency))}</span><span class="sales-line-rate"><input type="number" min="0" step="0.01" value="${esc(lineValue)}" placeholder="${inherited==null?'':esc(String(inherited))}" onchange="salesSetChargeOrderRate('${esc(line.id)}','${esc(row.key)}',this.value)"><small class="sales-rate-origin ${esc(st.origin)}">${esc(origin)}</small></span><strong>${final==null?'<span class="sales-price-missing">Rate required</span>':final.toFixed(2)+' '+esc(currency)}</strong><button class="sm" onclick="salesResetChargeRate('${esc(line.id)}','${esc(row.key)}')">Use Order / Catalog</button></div>`;}).join('')}</div><div class="sales-service-modal-total${summary.unpriced?' incomplete':''}"><span>Services / processing total for this line</span><strong>${summary.total.toFixed(2)} ${esc(currency)}</strong>${summary.unpriced?`<small>+ <span data-raw>${summary.unpriced}</span> <span>unpriced</span></small>`:''}</div>`:''}${salesLineCommercialAdjustmentsHtml(line,currency)}`:`<div class="empty compact sales-service-empty">This line has no services or processing charges yet.</div>`}</div></div>`;
}
...
function salesOrderServicesModal(){
 ...
 return `<div class="sales-service-modal-back" onclick="if(event.target===this)salesCloseServices()"><div class="sales-service-modal order-modal"><div class="sales-service-modal-head"><div><span>Sales Order</span><div class="sales-bulk-pricing-title"><h3>Services & Pricing</h3><span class="sales-bulk-pricing-badge">Bulk price edit</span></div><small>A rate changed in this window applies to every order line with the same service. Geometry and quantity do not change.</small></div><button onclick="salesCloseServices()">×</button></div>${groups.length||commercial?`${groups.length?`<div class="sales-bulk-pricing-banner"><b>Bulk edit</b><span>Changing an Order rate below applies it to all matching services in this order.</span></div>${summary.unpriced?`<div class="sales-pricing-warning"><b>Pricing incomplete</b><span><b data-raw>${summary.unpriced}</b> <span>charges have no effective rate. They are excluded from the order monetary total until a rate is entered.</span></span></div>`:''}<div class="sales-order-service-table"><div class="sales-order-service-row head"><span>Service / Processing</span><span>Order basis</span><span>Catalog rate</span><span>Order rate</span><span>Final total</span><span></span></div>${groups.map(function(g){const orderValue=g.orderRate==null?'':String(g.orderRate),placeholder=g.catalogRates.length===1?String(g.catalogRates[0]):'';return `<div class="sales-order-service-row${g.unpriced?' unpriced':''}"><span class="sales-price-charge"><b>${esc(g.label)}</b><small><span>Lines:</span> ${g.entries.length}${g.lineOverrides?' · <span>Overrides:</span> '+g.lineOverrides:''}${g.unpriced?' · <span class="sales-price-missing"><span>Unpriced:</span> <span data-raw>'+g.unpriced+'</span></span>':''}</small></span><span class="sales-price-basis">${esc(salesOrderGroupBasisText(g))}</span><span>${g.catalogRates.length?esc(salesOrderGroupCatalogText(g,currency)):'<span class="sales-price-missing">Not set</span>'}</span><span class="sales-order-rate"><input type="number" min="0" step="0.01" value="${esc(orderValue)}" placeholder="${esc(placeholder)}" onchange="salesSetOrderGroupRate('${esc(g.key)}',this.value)"><small class="sales-rate-origin ${g.orderRate!=null?'order':'catalog'}">${g.orderRate!=null?'ORDER':'CATALOG'}</small></span><strong>${g.unpriced&&g.total===0?'<span class="sales-price-missing">Rate required</span>':g.total.toFixed(2)+' '+esc(currency)+(g.unpriced?' + ?':'')}</strong><button class="sm" onclick="salesResetOrderGroupRate('${esc(g.key)}')">Reset to Catalog</button></div>`;}).join('')}</div><div class="sales-service-order-note"><b>Mass edit rule</b><span>Changing an Order rate applies it to all matching services in this order and clears older line exceptions for that service. A specific line can then be overridden from its Services cell.</span></div><div class="sales-service-modal-total grand${summary.unpriced?' incomplete':''}"><span>Services / processing total for the whole order</span><strong>${summary.total.toFixed(2)} ${esc(currency)}</strong>${summary.unpriced?`<small>+ <span data-raw>${summary.unpriced}</span> <span>unpriced</span></small>`:''}</div>`:''}${commercial}`:`<div class="empty compact sales-service-empty">No services or processing charges in this order yet.</div>`}</div></div>`;
}
