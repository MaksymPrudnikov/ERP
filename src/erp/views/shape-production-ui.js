...
function shapeProdBorderField(){
  ...
  /* The cutting hull may omit notch edges because they are fabricated after
     glass cutting. Keep every physical edge visible in the A–F strip anyway;
     omitted edges are explicit N/A cells instead of silently disappearing. */
  var shownEdges=plan.edges.slice(),shownBy={};
  shownEdges.forEach(function(e){shownBy[String(e.id)]=true;});
...
shapeManufacturingMarkersSvg=function(source,T){
  ...
  return shapeDimArrowDefs()+items.map(function(item,i){
    var chosen=item.id===sManufacturingSelected,selected=chosen?' selected':'',label=shapeMarkDrawingLabel(item);
    if(item.type==='hole'){
      var centers=shapeHoleCenters(item),px=centers.map(function(c){return [T.X(c[0]),T.Y(c[1])];}),x=px[0][0],y=px[0][1],d=fabParseDimStrict(item.diameter),dia=d.ok&&d.v>0?d.v:.75,r=Math.max(4,Math.min(14,dia*T.sc/2)),pos=shapeManufacturingHolePosition(item,{P:T.P,b:T.b})||{hRef:'left',vRef:'bottom'};
      var pair=shapeHoleInternalDimsSvg(item,px,T,chosen,pos.hRef==='right',pos.vRef==='top',r);
      return `<g class='shape-mi-marker hole${selected}' onclick='event.stopPropagation();sManufacturingSelected="${esc(item.id)}";sDimEdit=null;render()'>${px.map(function(c){return `<circle cx='${c[0]}' cy='${c[1]}' r='${r}'/>`;}).join('')}${pair}<text x='${x+r+8}' y='${y-8}'>${esc(label)} · ${px.length>1?px.length+' × ':''}Ø ${esc(shapeDrawingDim(dia))}</text></g>`;
    }
    var pt=shapeManufacturingEdgePoint(item,{P:T.P,b:T.b});if(!pt)return'';var e=pt.edge,x=T.X(pt.x),y=T.Y(pt.y),ang=Math.atan2(T.Y(e.end[1])-T.Y(e.start[1]),T.X(e.end[0])-T.X(e.start[0]))*180/Math.PI,mark=item.type==='clamp'?`<g transform='translate(${x} ${y}) rotate(${ang})'><rect x='-13' y='-13' width='26' height='26' rx='3'/><path d='M -5 -10 V 10 M 5 -10 V 10'/></g>`:`<g transform='translate(${x} ${y}) rotate(${ang})'><rect x='-16' y='-10' width='32' height='20' rx='3'/><line x1='0' y1='-10' x2='0' y2='10'/></g>`;return `<g class='shape-mi-marker ${item.type}${selected}' onclick='event.stopPropagation();sManufacturingSelected="${esc(item.id)}";render()'>${mark}<text data-raw x='${x+20}' y='${y-16}'>${esc(label+' · '+item.edgeId+' @ '+shapeDrawingDim16(shapeMiShownDistance(item,pt.edge&&pt.edge.len)))}</text></g>`;
  }).join('');
};
...
shapeMarksBodyHTML=function(){
  ...
  if(placing)body+=`<div class='shape-mi-place'><b>${placing.moveId?'Move':'Add'}: ${placing.type==='hole'?esc(shapeHoleName({count:placing.holeCount})):raw(shapeManufacturingItemTitle(placing.type))}</b><span>${placing.type==='hole'?'Click the first center inside the glass.':'Click near the exact DXF segment.'}</span><button class='sm' onclick='shapeCancelManufacturingPlacement()'>Cancel</button></div>`;
  body+=`<div class='shape-mi-list'>${items.length?items.map(function(item,i){
    ...
    if(item.type==='hole'){
      var d=fabParseDimStrict(item.diameter),pos=shapeManufacturingHolePosition(item,shapeManufacturingGeometry())||{hRef:'left',vRef:'bottom',hDistance:0,vDistance:0},count=shapeHoleCount(item);
      var pair=count===2?' · C-C '+shapeDim16(shapeHoleSpacing(item))+' · '+(shapeHoleAxis(item)==='vertical'?'Vertical':'Horizontal'):count===3?' · V C-C '+shapeDim16(shapeHoleTripleVSpacing(item))+' · H C-C '+shapeDim16(shapeHoleTripleHSpacing(item))+' · '+(shapeHoleTripleDirection(item)==='left'?'Left':'Right'):'';
      summary='Ø '+(d&&d.ok?dimIn16(d.v):item.diameter)+pair+' · '+shapeManufacturingEdgeLabel(pos.hRef)+' '+shapeDim16(pos.hDistance)+' · '+shapeManufacturingEdgeLabel(pos.vRef)+' '+shapeDim16(pos.vDistance);
      fields=shapeHolePairFieldsHTML(item)+`<div class='shape-mi-hole-position-grid'><div class='shape-mi-axis-card'><label>Horizontal reference<select onchange='shapeSetManufacturingHoleReference("${esc(item.id)}","h",this.value)'><option value='left' ${pos.hRef==='left'?'selected':''}>Left</option><option value='right' ${pos.hRef==='right'?'selected':''}>Right</option></select></label><label>${count>1?'Distance to first center':'Distance to center'}<input value='${esc(shapeFrac16(pos.hDistance))}' onchange='shapeSetManufacturingHoleDistance("${esc(item.id)}","h",this.value)'></label></div><div class='shape-mi-axis-card'><label>Vertical reference<select onchange='shapeSetManufacturingHoleReference("${esc(item.id)}","v",this.value)'><option value='bottom' ${pos.vRef==='bottom'?'selected':''}>Bottom</option><option value='top' ${pos.vRef==='top'?'selected':''}>Top</option></select></label><label>${count>1?'Distance to first center':'Distance to center'}<input value='${esc(shapeFrac16(pos.vDistance))}' onchange='shapeSetManufacturingHoleDistance("${esc(item.id)}","v",this.value)'></label></div></div><label>Diameter<input id='mi_d_${esc(item.id)}' value='${esc(item.diameter)}' oninput='shapeSetManufacturingField("${esc(item.id)}","diameter",this.value)' onchange='render()'></label>`+shapeDimControlsHTML(item.id,[{key:'h',label:'Horizontal'},{key:'v',label:'Vertical'}].concat(count===2?[{key:'c',label:'C-C'}]:count===3?[{key:'cv',label:'Vertical C-C'},{key:'ch',label:'Horizontal C-C'}]:[]));
    }else{
...
/* ---------- DXF Production / Cutting preview ----------
   Обе вкладки чертежа берут ОДНО поле и один масштаб: рамка считается по
   объединению готового контура, контура реза и safety border. Раньше каждая
   вкладка строила поле под себя, и при переключении Production <-> Cutting
   фигура прыгала и меняла размер. Габаритные размеры по-прежнему меряют
   ГОТОВЫЙ контур: x0/y0/dw/dh описывают именно его, а не объединение. */
function shapeProdDxfFrame(){
  var T=shapeDxfPreviewTransform(sDraft&&sDraft.source);if(!T)return null;
  var all=T.P.slice(),plan=ShapeModule.dxfCuttingPlan(sDraft);
  if(plan&&plan.valid)all=all.concat(plan.points||[]).concat(shapeBorderFramePoints(plan)||[]);
  var ub=fabEdgeBounds(all),UW=Math.max(.001,ub.maxX-ub.minX),UH=Math.max(.001,ub.maxY-ub.minY);
  var vw=T.vw,vh=T.vh,padL=88,padR=88,padT=62,padB=72;
  var sc=Math.min((vw-padL-padR)/UW,(vh-padT-padB)/UH),uw=UW*sc,uh=UH*sc;
  var ux=padL+(vw-padL-padR-uw)/2,uy=padT+(vh-padT-padB-uh)/2;
  var X=function(x){return ux+(x-ub.minX)*sc;},Y=function(y){return uy+uh-(y-ub.minY)*sc;};
  return {P:T.P,b:T.b,W:T.W,H:T.H,vw:vw,vh:vh,sc:sc,X:X,Y:Y,plan:plan,
    x0:X(T.b.minX),y0:Y(T.b.maxY),dw:T.W*sc,dh:T.H*sc};
}
function shapeProdDxfProductionSvg(){
  var result=ShapeModule.compute(sDraft),T=shapeProdDxfFrame();if(!T)return `<div class='module-invalid'>DXF preview unavailable</div>`;
  ...
  var markers=shapeManufacturingMarkersSvg(sDraft.source,T)+shapeStampOverlaySvg(T),topY=Math.max(20,T.y0-24),leftX=Math.max(24,T.x0-26);
  return `<div class='shape-prod-dxf-card'>${sManufacturingPlace?`<div class='shape-prod-place-note'><b>Placement mode:</b> click ${sManufacturingPlace.type==='hole'?'inside the finished contour':'near the required physical SEG'}.</div>`:''}<svg class='shape-dxf-svg shape-prod-dxf-svg${placing}' viewBox='0 0 ${T.vw} ${T.vh}' ${sManufacturingPlace?"onclick='shapePlaceManufacturingFromEvent(event,this)'":''}><defs><marker id='shapeProdArrow' viewBox='0 0 8 8' refX='8' refY='4' markerWidth='5' markerHeight='5' orient='auto-start-reverse'><path d='M0,0 L8,4 L0,8 Z' fill='#d92d20'/></marker></defs><path d='${path}' fill='rgba(46,144,250,.04)' stroke='#667085' stroke-width='1.5'/>${markers}${edges}<line x1='${T.x0}' y1='${topY}' x2='${T.x0+T.dw}' y2='${topY}' class='shape-dxf-dim-line' marker-start='url(#shapeProdArrow)' marker-end='url(#shapeProdArrow)'/><line x1='${T.x0}' y1='${topY-5}' x2='${T.x0}' y2='${topY+5}' class='shape-dxf-dim-line'/><line x1='${T.x0+T.dw}' y1='${topY-5}' x2='${T.x0+T.dw}' y2='${topY+5}' class='shape-dxf-dim-line'/><text x='${T.x0+T.dw/2}' y='${topY-9}' class='shape-dxf-dim-text' text-anchor='middle'>${esc(shapeDrawingDim(T.W))}</text><line x1='${leftX}' y1='${T.y0}' x2='${leftX}' y2='${T.y0+T.dh}' class='shape-dxf-dim-line' marker-start='url(#shapeProdArrow)' marker-end='url(#shapeProdArrow)'/><line x1='${leftX-5}' y1='${T.y0}' x2='${leftX+5}' y2='${T.y0}' class='shape-dxf-dim-line'/><line x1='${leftX-5}' y1='${T.y0+T.dh}' x2='${leftX+5}' y2='${T.y0+T.dh}' class='shape-dxf-dim-line'/><text x='${leftX-10}' y='${T.y0+T.dh/2}' class='shape-dxf-dim-text' text-anchor='middle' transform='rotate(-90 ${leftX-10} ${T.y0+T.dh/2})'>${esc(shapeDrawingDim(T.H))}</text></svg><div class='shape-prod-dxf-help'><b>FINISHED contour</b><span>Edge processing derives Cutting. Manufacturing Items do not change the outer cutting contour.</span></div></div>`;
}
function shapeProdDxfCuttingSvg(){
  ...
  var T=shapeProdDxfFrame();if(!T)return `<div class='module-invalid'>DXF preview unavailable</div>`;
  var finished=plan.finishedPoints,cut=plan.points,vw=T.vw,vh=T.vh,X=T.X,Y=T.Y,path=function(P){return P.map(function(q,i){return (i?'L':'M')+X(q[0]).toFixed(2)+' '+Y(q[1]).toFixed(2);}).join(' ')+' Z';},cb=fabEdgeBounds(cut);
...
shapeForm=function(){
  ...
  var workspaceTabs=`<div class='shape-workspace-tabs' role='tablist' aria-label='Shape workflow'><button type='button' role='tab' aria-selected='${sWorkspaceTab==='designer'}' class='${sWorkspaceTab==='designer'?'on':''}' onclick='setShapeWorkspaceTab("designer")'><span>1</span><b>Shape Designer</b><small>Geometry · Lites · Edge processing</small></button><button type='button' role='tab' aria-selected='${sWorkspaceTab==='cutout'}' class='${sWorkspaceTab==='cutout'?'on':''}' onclick='setShapeWorkspaceTab("cutout")'><span>2</span><b>Fabrication</b><small>Hole · Hardware · Stamp · Sandblast · Cut shape</small>${cutoutCount?`<i>${cutoutCount}</i>`:''}</button></div>`;
  var tabs=external
    ? `<div class='shape-view-tabs'><button class='${sView!=='cutting'?'on':''}' onclick='setShapeView("production")'>Production Drawing</button><button class='${sView==='cutting'?'on':''}' onclick='setShapeView("cutting")'>Cutting DXF</button>${shapeMetricToggleButton(true)}<button class='shape-print-btn' onclick='shapePrintDrawing()'>Print / PDF</button></div>`
