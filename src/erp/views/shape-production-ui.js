...
shapeManufacturingMarkersSvg=function(source,T){
  ...
  return items.map(function(item,i){var selected=item.id===sManufacturingSelected?' selected':'',label=shapeMarkDrawingLabel(item);if(item.type==='hole'){var centers=shapeHoleCenters(item),px=centers.map(function(c){return [T.X(c[0]),T.Y(c[1])];}),x=px[0][0],y=px[0][1],d=fabParseDimStrict(item.diameter),dia=d.ok&&d.v>0?d.v:.75,r=Math.max(4,Math.min(14,dia*T.sc/2)),pair=px.length===2?`<line x1='${px[0][0]}' y1='${px[0][1]}' x2='${px[1][0]}' y2='${px[1][1]}' class='shape-hole-pair-line'/><text x='${(px[0][0]+px[1][0])/2}' y='${(px[0][1]+px[1][1])/2-10}' text-anchor='middle'>${esc(shapeDim16(shapeHoleSpacing(item))+' C-C')}</text>`:'';return `<g class='shape-mi-marker hole${selected}' onclick='event.stopPropagation();sManufacturingSelected="${esc(item.id)}";render()'>${px.map(function(c){return `<circle cx='${c[0]}' cy='${c[1]}' r='${r}'/>`;}).join('')}${pair}<text x='${x+r+8}' y='${y-8}'>${esc(label)} · ${px.length===2?'2 × ':''}Ø ${esc(dimIn16(dia))}</text></g>`;}var pt=shapeManufacturingEdgePoint(item,{P:T.P,b:T.b});if(!pt)return'';var e=pt.edge,x=T.X(pt.x),y=T.Y(pt.y),ang=Math.atan2(T.Y(e.end[1])-T.Y(e.start[1]),T.X(e.end[0])-T.X(e.start[0]))*180/Math.PI,mark=item.type==='clamp'?`<g transform='translate(${x} ${y}) rotate(${ang})'><rect x='-13' y='-13' width='26' height='26' rx='3'/><path d='M -5 -10 V 10 M 5 -10 V 10'/></g>`:`<g transform='translate(${x} ${y}) rotate(${ang})'><rect x='-16' y='-10' width='32' height='20' rx='3'/><line x1='0' y1='-10' x2='0' y2='10'/></g>`;return `<g class='shape-mi-marker ${item.type}${selected}' onclick='event.stopPropagation();sManufacturingSelected="${esc(item.id)}";render()'>${mark}<text data-raw x='${x+20}' y='${y-16}'>${esc(label+' · '+item.edgeId+' @ '+shapeDim16(shapeMiShownDistance(item,pt.edge&&pt.edge.len)))}</text></g>`;}).join('');
};
...
shapeMarksBodyHTML=function(){
  ...
  body+=`<div class='shape-mi-list'>${items.length?items.map(function(item,i){
    ...
    if(item.type==='hole'){
      ...
      summary='Ø '+(d&&d.ok?dimIn16(d.v):item.diameter)+(shapeHoleCount(item)===2?' · C-C '+shapeDim16(shapeHoleSpacing(item))+' · '+(shapeHoleAxis(item)==='vertical'?'Vertical':'Horizontal'):'')+' · '+shapeManufacturingEdgeLabel(pos.hRef)+' '+shapeDim16(pos.hDistance)+' · '+shapeManufacturingEdgeLabel(pos.vRef)+' '+shapeDim16(pos.vDistance);
      fields=shapeHolePairFieldsHTML(item)+`<div class='shape-mi-hole-position-grid'><div class='shape-mi-axis-card'><label>Horizontal reference<select onchange='shapeSetManufacturingHoleReference("${esc(item.id)}","h",this.value)'><option value='left' ${pos.hRef==='left'?'selected':''}>Left</option><option value='right' ${pos.hRef==='right'?'selected':''}>Right</option></select></label><label>Distance to first center<input value='${esc(shapeFrac16(pos.hDistance))}' onchange='shapeSetManufacturingHoleDistance("${esc(item.id)}","h",this.value)'></label></div><div class='shape-mi-axis-card'><label>Vertical reference<select onchange='shapeSetManufacturingHoleReference("${esc(item.id)}","v",this.value)'><option value='bottom' ${pos.vRef==='bottom'?'selected':''}>Bottom</option><option value='top' ${pos.vRef==='top'?'selected':''}>Top</option></select></label><label>Distance to first center<input value='${esc(shapeFrac16(pos.vDistance))}' onchange='shapeSetManufacturingHoleDistance("${esc(item.id)}","v",this.value)'></label></div></div><label>Diameter<input id='mi_d_${esc(item.id)}' value='${esc(item.diameter)}' oninput='shapeSetManufacturingField("${esc(item.id)}","diameter",this.value)' onchange='render()'></label>`;
    }else{
