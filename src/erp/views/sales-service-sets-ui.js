...
function salesApplyServiceAssignment(){
  ...
  var set=salesServiceSetById(soDraft,p.setId);
  soDraft.lines.forEach(function(line){if(!ids.has(line.id)||salesLineLocked(line))return;line.serviceSetId='';salesApplySetOpsToShape(line,set);});
  touch();
...
function salesOrderScreenColumns(){
  var metrics=new Set(salesMetricColumnsFor('screen').map(function(c){return c.key;})),base=new Set(SALES_ORDER_BASE_COLUMNS.map(function(c){return c.key;}));
  return salesOrderColumnOrder().filter(function(key){return base.has(key)||metrics.has(key);});
}
function salesOrderColumnHeader(key){
  var metric=SALES_METRIC_COLUMNS.find(function(c){return c.key===key;});
  if(metric)return `<th class="line-metric" data-metric="${metric.key}"><span data-raw>${metric.label}</span></th>`;
  var base=SALES_ORDER_BASE_COLUMNS.find(function(c){return c.key===key;});return `<th data-column="${esc(key)}">${esc(base?base.label:key)}</th>`;
}
function salesOrderColumnCell(key,l,i,linked,st){
  var metric=SALES_METRIC_COLUMNS.find(function(c){return c.key===key;});if(metric)return salesMetricCell(l,metric,'screen');
  if(key==='mu')return `<td><select class='line-mu' onchange='soDraft.lines[${i}].makeupId=this.value;render()'>${soDraft.makeups.map(function(m){return `<option value='${esc(m.id)}' ${l.makeupId===m.id?'selected':''}>${esc(m.code)}</option>`;}).join('')}</select></td>`;
  if(key==='set')return `<td class='ss-cell'>${salesSetBadge(l)}</td>`;
  if(key==='qty')return `<td><input class='line-qty' type='number' min='1' step='1' value='${esc(l.qty)}' oninput='soDraft.lines[${i}].qty=salesPositiveInt(this.value,1)' onchange='render()'></td>`;
...
function salesOrderLines(){
  var entries=salesServiceFilteredEntries(),visible=entries.map(function(x){return x.line;}),allSelected=visible.length>0&&visible.every(function(l){return soSelectedLines.has(l.id);}),currency=soDraft.currency||'CAD',pricing=salesOrderPricingSummary(),commercial=salesOrderCommercialAdjustments(soDraft),commercialLines=commercial.reduce(function(n,g){return n+g.lines;},0),hasServices=pricing.charges>0||commercial.length>0,columns=salesOrderScreenColumns();
  return `<section class='sales-block sales-lines-block'><div class='sales-lines-head'><div><b>ORDER LINES</b><span>Drag and reorder columns in Columns</span></div><div class='sales-line-tools'>${salesMetricsTools()}<button class='sales-services-order-btn ${hasServices?'has-services':''}${pricing.unpriced?' incomplete':''}' onclick='salesOpenOrderServices()'><span>Services & prices</span><span class='sales-order-price-summary'><b>${pricing.total.toFixed(2)} ${esc(currency)}</b>${pricing.unpriced?`<small>${pricing.unpriced} no rate</small>`:(commercial.length?`<small>${commercialLines} price adjustment${commercialLines===1?'':'s'}</small>`:'')}</span></button><button class='ss-order-btn' onclick='salesOpenServiceSets()'>Edgework Sets <span>${(soDraft.serviceSets||[]).length}</span></button><button onclick='salesExcelOpen()'>Paste Excel</button><button onclick='salesOrderAddTen()'>+10</button><button class='pri' onclick='salesOrderAddLine(null,true)'>+ Line</button></div></div>${salesServiceFiltersHtml()}${salesBulkServiceControls()}<div class='sales-table-wrap'><table class='sales-lines-table sales-lines-with-services'><thead><tr><th class='line-check'><input type='checkbox' ${allSelected?'checked':''} onchange='salesToggleAllLines(this.checked)'></th><th>#</th>${columns.map(salesOrderColumnHeader).join('')}<th></th></tr></thead><tbody>${entries.map(function(x){var l=x.line,i=x.index,linked=!salesLineOwnsDimensions(l),st=salesLineServiceStatus(l);return `<tr data-metrics-line-id="${esc(l.id)}"${salesLineRowAttrs(l)}>${salesLockedCell(l,`<td class='line-check'><input type='checkbox' ${soSelectedLines.has(l.id)?'checked':''} onchange='salesToggleLine("${esc(l.id)}",this.checked)'></td>`)}${salesLockedCell(l,`<td class='mono line-num'>${i+1}${salesLineBadge(l)}</td>`)}${columns.map(function(key){return salesLockedCell(l,salesOrderColumnCell(key,l,i,linked,st),key);}).join('')}${salesLockedCell(l,`<td><button class='sm dl line-delete' onclick='salesOrderRemoveLine(${i})'>×</button></td>`)}</tr>`;}).join('')||`<tr><td colspan='${columns.length+3}' class='empty'>No lines in this filter.</td></tr>`}</tbody></table></div>${salesCommercialOrderSummary()}</section>`;
}
...
function salesSetLineEdgeOp(lineId,edgeId,type,on){var line=soDraft.lines.find(function(l){return l.id===lineId;});if(!line||salesLockedLineGuard(line))return;var r=salesApplyLineEdgeOperation(line,edgeId,type,on);if(!r.ok)alert(r.reason);render();}
...
function salesSetLineOpParam(lineId,edgeId,type,key,value){
  var line=soDraft.lines.find(function(l){return l.id===lineId;}),shape=line&&salesLineGeometryShape(line);if(!line||!shape||salesLockedLineGuard(line))return;if(!line.serviceOverrides)line.serviceOverrides={pinnedTopology:'',edges:{}};
...
/* Превью показывает три вещи разом: готовую деталь, контур реза (больше на
   припуск кромки) и зону Safety Border вокруг реза. Раньше оба контура рисовались
   без стилей, а у SVG заливка по умолчанию чёрная — окно показывало чёрный
   прямоугольник вместо чертежа. */
function salesEffectivePreviewSvg(plan){
  if(!plan||!plan.valid||!(plan.finishedPoints||[]).length)return '';
  /* Бордер лежит НЕ вокруг всей детали, а только со стороны скошенной или
     дуговой кромки — рисуем его по тем же per-side отступам, по которым
     считается оплачиваемый габарит, иначе картинка врёт. */
  var pad=(plan.footprint&&plan.footprint.pad)||{left:0,right:0,top:0,bottom:0};
  var applies=!!(plan.safetyBorder&&plan.safetyBorder.applies);
  var all=(plan.finishedPoints||[]).concat(plan.cuttingPoints||[]),cb=fabEdgeBounds(plan.cuttingPoints||all);
  var b=fabEdgeBounds(all);
  if(applies)b={minX:Math.min(b.minX,cb.minX-(+pad.left||0)),maxX:Math.max(b.maxX,cb.maxX+(+pad.right||0)),minY:Math.min(b.minY,cb.minY-(+pad.bottom||0)),maxY:Math.max(b.maxY,cb.maxY+(+pad.top||0))};
  var W=Math.max(.01,b.maxX-b.minX),H=Math.max(.01,b.maxY-b.minY),vw=640,vh=420,p=45,sc=Math.min((vw-p*2)/W,(vh-p*2)/H);
  var X=function(x){return p+(x-b.minX)*sc;},Y=function(y){return vh-p-(y-b.minY)*sc;};
  var path=function(P){return P.map(function(q,i){return (i?'L ':'M ')+X(q[0])+' '+Y(q[1]);}).join(' ')+' Z';};
  var border=applies?`<rect class='border-zone' x='${X(cb.minX-(+pad.left||0))}' y='${Y(cb.maxY+(+pad.top||0))}' width='${(cb.maxX-cb.minX+(+pad.left||0)+(+pad.right||0))*sc}' height='${(cb.maxY-cb.minY+(+pad.top||0)+(+pad.bottom||0))*sc}'/>`:'';
  return `<svg class='ss-effective-svg' viewBox='0 0 ${vw} ${vh}'>${border}<path d='${path(plan.cuttingPoints)}' class='cutting'/><path d='${path(plan.finishedPoints)}' class='finished'/></svg>`;
}
/* Размеры словами: припуск кромки и бордер на глаз по картинке не читаются. */
function salesEffectiveSizesHtml(plan,line){
  if(!plan||!plan.valid)return '';
  var fp=plan.footprint||{},brd=plan.safetyBorder||{},applies=!!brd.applies&&(+brd.value||0)>0;
  return `<div class='ss-effective-sizes'>`
   +`<div><span>Finished</span><b>${esc(dimIn(plan.finishedW)+' × '+dimIn(plan.finishedH))}</b></div>`
   +`<div><span>Cutting</span><b>${esc(dimIn(plan.cutW)+' × '+dimIn(plan.cutH))}</b><small>edgework allowance</small></div>`
   +`<div class='${applies?'':'mut'}'><span>Billable footprint</span><b>${esc(dimIn(+fp.width||plan.cutW)+' × '+dimIn(+fp.height||plan.cutH))}</b><small>${applies?'safety border '+esc(dimIn(+brd.value||0))+' · client pays for it':'no safety border on straight edges'}</small></div>`
   +`<div><span>Thickness</span><b>${esc(String(plan.thickness))} mm</b><small>from Makeup, not from Shape</small></div></div>`
   +salesEffectiveLitesHtml(plan,line);
}
/* Пакет — это несколько стёкол, и у каждого своя кромка, свой припуск и свой
   размер реза. Показываем раскладку по лайтам: без неё комбинация 10 + 6
   выглядела бы одним куском, как раньше и считалось. */
function salesEffectiveLitesHtml(plan,line){
 var lites=plan&&plan.lites||[];
 if(!lites.length)return '';
 var m=line&&soDraft?salesMakeupById(soDraft,line.makeupId):null;
 return `<div class='ss-effective-lites'><span class='ss-lites-title'>Lites · edgework of each glass</span>`
  +lites.map(function(l){
    var ops=[],pane=m&&m.panes[l.index],auto=pane?salesPaneAutoEdgework(pane,m.unitType):'';
    (l.groups||[]).forEach(function(g){g.ops.forEach(function(o){if(ops.indexOf(o.type)<0)ops.push(o.type);});});
    /* Выбор кромки лайта живёт здесь же: именно тут видно, что у одного стекла
       полировка, а у другого арис, и здесь же это меняется. */
    /* Управление кромкой и отступом лайта живёт в форме строки — здесь только
       результат, чтобы окно не стало вторым местом правки. */
    /* Правится всё в форме — здесь только видно, чья геометрия у лайта, и есть
       короткий путь к ней. */
    var picker=line?`<span class='ss-lite-shape'>${l.ownShape?`<b data-raw>${esc(l.shapeName||'own shape')}</b>`:'<i>shared shape</i>'}${l.mirrored?`<b class='ss-lite-mirror'>mirrored</b>`:''}<button class='sm' onclick='salesOpenLiteShape("${esc(line.id)}",${l.index})'>${l.ownShape?'Open':'Own shape'}</button></span>`:'';
    return `<div class='ss-lite-row'>${shapeMiniContourSvg(l.finishedPoints,l.mirrored?'mirrored':'')}<b>${esc(l.label)}</b><span class='mm'>${esc(l.thickness)} mm</span>${picker}<span class='ops'>${esc(ops.join(' · ')||'no edgework')}</span><span class='cut'>cut ${esc(dimIn(l.cutW)+' × '+dimIn(l.cutH))}</span></div>`;
   }).join('')
  +`<small>${lites.length<2?'Single lite — edgework comes from its own glass.':(plan.uniformCut?'Both lites cut to the same size.':'Lites are cut to different sizes — each has its own allowance.')} Edgework set on the Shape stays above this choice.</small></div>`;
}
...
function salesDownloadEffectiveCutting(lineId,kind,liteIndex){
  var line=soDraft.lines.find(function(l){return l.id===lineId;}),payload=line&&salesEffectiveMachinePayload(line);if(!payload)return;if(!payload.ok)return alert(payload.reason);
  var base=salesEffectiveFileBase(line),points=payload.outer.points;
  if(liteIndex!=null){
    var lite=(payload.lites||[]).find(function(x){return x.index===liteIndex;});
    if(!lite)return alert('Lite is unavailable.');
    points=lite.outer.points;base=base+'_LITE'+(liteIndex+1);
  }
...
function salesEffectiveMachineCard(line,shape,plan){var issue=salesEffectiveMachineIssue(line,shape,plan);if(issue)return `<div class='ss-machine blocked'><b>ORDER EFFECTIVE · machine cutting</b><span>BLOCKED · ${esc(issue)}</span></div>`;return `<div class='ss-machine ok'><div><b>ORDER EFFECTIVE · machine cutting</b><small>Same Effective edge map as Services & prices.</small></div><code>${esc(salesEffectiveProductionFingerprint(line,shape,plan))}</code><div>${salesEffectiveCuttingButtons(line,plan)}</div></div>`;}
/* Когда лайты режутся по-разному, один файл на строку — это неправда: у каждого
   стекла свой контур. Тогда даём по паре файлов на лайт. */
function salesEffectiveCuttingButtons(line,plan){
  var lites=(plan&&plan.lites)||[];
  if(lites.length<2||plan.uniformCut)return `<button onclick='salesDownloadEffectiveCutting("${esc(line.id)}","svg")'>Effective Cutting SVG</button><button onclick='salesDownloadEffectiveCutting("${esc(line.id)}","dxf")'>Effective Generic DXF</button>`;
  return lites.map(function(l){
    return `<span class='ss-machine-lite'><b>${esc(l.label)} · ${esc(l.thickness)} mm</b><button onclick='salesDownloadEffectiveCutting("${esc(line.id)}","svg",${l.index})'>SVG</button><button onclick='salesDownloadEffectiveCutting("${esc(line.id)}","dxf",${l.index})'>DXF</button></span>`;
  }).join('');}
/* Припуск стороны: показываем, что предложила таблица, и даём поменять.
   Пустое поле = «как предлагает таблица». Владелец просил именно это: у
   склейки 6+10 плиты доводят по-разному, и «этот припуск мы обсуждаем каждый
   раз» — теперь обсуждение заканчивается в строке заказа, а не на словах. */
function salesEdgeAllowanceCell(line,g){
...
/* Плиты разной толщины дают разные припуски, а рез у склейки один. Предлагаем
   ...
   решение за цехом, и правится оно тут же, по сторонам. */
function salesLiteAllowanceNote(plan){
  var vary=(plan&&plan.lites||[]).filter(function(l){return l.pliesVary;});
...
function salesLineEdgeworkModal(){
  ...
  var warning=!snap.valid?`<div class='ss-line-warning bad'>${esc(snap.reason)}</div>`:snap.mappingPending?`<div class='ss-line-warning warn'><b>${esc(set&&set.code||'Bulk Set')} is pending mapping.</b> Shape-owned processing remains active in Services; machine release is blocked until mapping is confirmed.</div>`:!plan.valid?`<div class='ss-line-warning bad'>${esc(plan.reason)}</div>`:lost.length?`<div class='ss-line-warning bad'>Lost override edge(s): ${esc(lost.join(', '))}</div>`:`<div class='ss-line-warning info'><b>Order Effective</b> = edgework of this line's Shape, base <b>Rough Arris / Flat Polish</b> from the glass of its Makeup where the Shape says nothing, and Makeup thickness for the cutting allowance. Nothing is stored on the line, so nothing can go stale.<button class='sm' onclick='salesOpenShapeFromEdgework("\${esc(line.id)}")'>Open Shape</button></div>`;
  /* Габарит станции — поверх обычного пояснения, а не вместо него: рез при этом
     не заблокирован, работа идёт, но деталь не влезет в станцию маршрута. */
  var sizeProblems=plan.valid&&typeof salesStationSizeProblems==='function'?salesStationSizeProblems(line,soDraft):[];
  if(sizeProblems.length)warning=`<div class='ss-line-warning warn'><b>Does not fit a station of its route</b>${salesStationSizeTexts(sizeProblems).map(function(t){return `<div>${esc(t)}</div>`;}).join('')}<small>A station limit never reroutes the work: it only warns before cutting. Station sizes are edited in Production → Stations, a work's own limit in Master Data → Works.</small></div>`+warning;
  /* Припуск проставляется при расчёте реза, поэтому карточки берут группы из
     ПЛАНА: у снимка это поле ещё пустое, и колонка припуска не показывалась
     вовсе. Снимок остаётся запасным вариантом, когда рез заблокирован. */
  var groups=(plan&&plan.valid&&plan.groups&&plan.groups.length?plan.groups:snap.groups)||[];
  return `<div class='sales-service-modal-back' onclick='if(event.target===this)salesCloseLineEdgework()'><div class='sales-service-modal line-modal'><div class='sales-service-modal-head'><div><span>Sales Order line ${esc(line.mark||line.id)}</span><h3>Effective Production</h3><small>${line.shapeRef&&line.shapeRef.id?esc(shape.name):'Width × Height rectangle'}${set?' · '+esc(set.code):''}</small></div><button onclick='salesCloseLineEdgework()'>×</button></div>${warning}${salesLiteAllowanceNote(plan)}<div class='ss-effective-workspace'><div class='ss-effective-visual'>${plan.valid?salesEffectivePreviewSvg(plan):`<div class='ss-cut-block'><b>CUTTING BLOCKED</b><span>${esc(plan.reason||snap.reason||'Unresolved')}</span></div>`}<div class='ss-effective-legend'><span class='lg-finished'>Finished</span><span class='lg-cutting'>Cutting</span><span class='lg-border'>Safety border</span></div>${plan.valid?salesEffectiveSizesHtml(plan,line):''}</div><div class='ss-effective-edges'>${groups.map(function(g){var ov=Object.prototype.hasOwnProperty.call((line.serviceOverrides&&line.serviceOverrides.edges)||{},g.id);return `<div class='ss-effective-edge'><div class='ss-effective-edge-head'><div><b>${esc(g.id.toUpperCase())}</b><small>${esc(dimIn(g.length))} · ${esc(g.side==='other'?'OTHER':g.side)}</small></div><div><span class='ss-source${g.liteVaries?' varies':''}'>${esc(g.liteVaries?g.source+' · varies by lite':g.source)}</span>${salesEdgeAllowanceCell(line,g)}</div></div><div class='ss-effective-opset read-only'>${(function(){var picked=SHAPE_EDGE_OPS.filter(function(type){return (g.effectiveOps||g.ops).some(function(o){return o.type===type;});});
      /* Полоса только читается, поэтому невыбранные операции здесь ничего не
         сообщают — семь серых чипов на одну активную разносили карточку на две
         строки. Показываем то, что реально уйдёт в цех. */
      return picked.length?picked.map(function(type){return `<span class='ss-op on'>${esc(salesServiceShort(type))}</span>`;}).join(''):`<span class='ss-op none'>no edgework</span>`;})()}</div></div>`;}).join('')||`<div class='empty compact'>No physical edges available.</div>`}</div></div>${dxf&&set&&set.mode==='sides'?salesDxfMappingHtml(line,shape):''}${salesEffectiveMachineCard(line,shape,plan)}<div class='ss-line-actions'>${salesHasLineEdgeOverrides(line)?`<button onclick='salesRevertAllEdges("${esc(line.id)}")'>Clear all line overrides</button>`:''}${salesHasEdgeAllowanceOverrides(line)?`<button onclick='salesResetEdgeAllowances("${esc(line.id)}")'>Reset allowances</button>`:''}${lost.length?`<button class='dl' onclick='salesRemoveLostEdges("${esc(line.id)}")'>Remove lost overrides</button>`:''}${line.shapeRef&&line.shapeRef.id?`<button onclick='salesOpenShapeFromEdgework("${esc(line.id)}")'>Open Shape · standalone</button>`:''}<button class='pri' onclick='salesOpenLineServices("${esc(line.id)}")'>Open Services · same snapshot</button></div></div></div>`;
}
...
salesOrderNew=function(kind){salesResetServiceSetUi();return __salesServiceOrderNew(kind);};
