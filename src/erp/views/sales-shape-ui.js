...
function viewShapeSkill(){
 const rows=DB.shapeDef.map((s,i)=>{const r=ShapeModule.compute(s);return `<tr><td><b>${raw(s.name)}</b></td><td class="mono">${r.valid?dimIn(r.width)+' × '+dimIn(r.height):'<span class="bad pill">невалидна</span>'}</td><td class="mono">${r.valid?r.points.length:'—'}</td><td>${r.valid?'<span class="pill ok">Smart-Shape</span>':'<span class="pill bad">'+esc(r.reason)+'</span>'}</td><td style="white-space:nowrap"><button class="sm" onclick="openShapeEdit(${i})">Изменить</button><button class="sm dl" onclick="delShape(${i})">×</button></td></tr>`;}).join('');
...
function shapePreviewSVG(r){
 if(!r.valid)return `<div class="module-invalid">${esc(r.reason||'Invalid shape')}</div>`;
...
function saveShape(){const e=document.getElementById('e_shape');e.style.display='none';sDraft.name=(sDraft.name||'').trim();if(!sDraft.name)return fail(e,'Укажи название');const r=ShapeModule.compute(sDraft);if(!r.valid)return fail(e,r.reason);if(sEdit==='new')DB.shapeDef.push(sDraft);else DB.shapeDef[sEdit]=sDraft;sEdit=null;sDraft=null;touch();render();}
