...
/* Размеры в шапке. У Smart-Shape те же A, B и C есть и в матрице рёбер — это
   осознанное повторение: шапка заполняется при заведении формы, матрица служит
   для правки по сторонам. Поля синхронизируются в refreshShapeEditor, поэтому
   разойтись не могут. */
function shapeMasterSizeFields(){
  ...
  var c=sDraft.type==='smart'?`<div><label>C · Right height</label><input id='shapeCField' data-live-placeholder value='${esc(sDraft.smart.C.len||'')}' placeholder='${esc(shapeCEffective())}' oninput='setShapeC(this.value)'></div>`:'';
  return w+h+c;
}
