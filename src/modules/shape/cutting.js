...
function shapeFillNotches(points,dist,ids){
  ...
  while(guard++<400){
    ...
    if(!hit)break;
    /* Припуск снятых кромок НЕ поднимается в оставшиеся. Нотч выпиливают уже
       после реза и после кромки, от обработанного края: шлифовка его рёбер
       снимает материал внутри самого выреза, а не с наружного контура. Пока
       припуск поднимался, CNC на рёбрах нотча раздувал всю заготовку и
       заставлял обрабатывать по CNC-припуску целые стороны фигуры. */
    ...
    /* Ребро, которое начинается в hit, — это ПРОДОЛЖЕНИЕ кромки pp: она дала
       вторую линию пересечения, её же припуск взят строкой выше. Раньше сюда
       попадал id снятой кромки нотча, и в Safety Border реальная сторона
       (например нижняя B) исчезала как «N/A», а её строку занимало ребро
       нотча, которого на контуре реза нет вовсе. */
    keepP.push(hit);keepD.push(+D[pp]||0);keepI.push(I[pp]!=null?I[pp]:I[pm]);
