...
> Текущая точка 25 сентября 2026: очередь Optimization, батчи с Glass ID, NCR/Recut, стикеры и собственный раскрой с ручными правками. Раскрой учитывает безопасные сквозные резы, автоматический фактический левый Trim Y на отдельном листе, группы одинаковых схем и очередь размеров сырья. Кнопка Trial export выдаёт **отдельные файлы, не ZIP**, для одного открытого листа с прямоугольниками и фигурами — прямые и дуги, эллипс цепочкой дуг: Maver `.ISO`/`.BMP`, Disai `.dst`/`.sum`. Можно сохранить пару в выбранную папку, если браузер поддерживает эту возможность. Disai `.dst` пишет полную схему X/Y/Z/U/V/W/R (это уровни схемы, не оси стола), правила сверены со всеми 189 схемами образцов Perfect Cut; проект Disai — папка `<имя>_<код стекла>.prjx`. Это **только для проверки без запуска реза**; производственный экспорт ещё не готов. 845 проверок для `src` и 845 для `dist`. Точное состояние и ограничения — в [`docs/GLASS_ERP_HANDOFF.md`](docs/GLASS_ERP_HANDOFF.md) и [`docs/MACHINE_EXPORT_STATUS.md`](docs/MACHINE_EXPORT_STATUS.md).
