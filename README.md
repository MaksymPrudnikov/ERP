...
**Что произойдёт дальше — само.** GitHub Action (`.github/workflows/build.yml`) проверит, что все модули зарегистрированы, пересоберёт `dist/GLASS_ERP.html` и прогонит 38 тестов. Зелёная галочка рядом с коммитом — живая версия обновилась. Красный крестик — тесты не прошли, `dist` НЕ обновлён, работающая система осталась на прежнем коде; открой вкладку **Actions**, там написано, какой именно тест упал.
