...
> Текущая точка 25 сентября 2026: очередь Optimization, батчи с Glass ID, NCR/Recut, стикеры и собственный раскрой с ручными правками. Раскрой учитывает безопасные сквозные резы, автоматический фактический левый Trim Y на отдельном листе, группы одинаковых схем и очередь размеров сырья. Иконка выгрузки в шапке Optimization → Maver или Disai пишет пару станочных файлов, **не ZIP**, для одного открытого листа с прямоугольниками и фигурами — прямые и дуги, эллипс цепочкой дуг: Maver `.ISO`/`.BMP`, Disai `.dst`/`.sum` — в выбранную папку (без выбора папки в браузере — обычной загрузкой). Disai `.dst` пишет полную схему X/Y/Z/U/V/W/R (это уровни схемы, не оси стола), правила сверены со всеми 196 схемами образцов Perfect Cut; проект Disai — папка `<имя>_<код стекла>.prjx`. Это **только для проверки без запуска реза**; производственный экспорт ещё не готов. 851 проверка для `src` и 851 для `dist`. Точное состояние и ограничения — в [`docs/GLASS_ERP_HANDOFF.md`](docs/GLASS_ERP_HANDOFF.md) и [`docs/MACHINE_EXPORT_STATUS.md`](docs/MACHINE_EXPORT_STATUS.md).
...
Текущая стадия: **Фаза 2, собственный раскрой и подготовка машинного экспорта**. Customer Master, Draft Sales Orders, IGU Makeups и Production Shape уже существуют; перед выдачей программ на столы требуется проверка форматов и контроллеров.
...
| «в интерфейсе осталось русское слово» | соответствующий `src/erp/views/*.js` — интерфейс только по-английски, словаря `i18n.js` больше нет |
...
Services строки включают `Shape Unit` — 1.25 CAD за Billable ft² для любой
формы, кроме обычного Rectangle; внешний DXF считается Shape. Площадь
округляется и учитывает минимум к оплате. Triple и Large unit теперь являются
процентными надбавками от цены готового юнита с сервисами; каждая причина
показана отдельно в детализации Unit Price / Line Total, короткой меткой в
Services и отдельной строкой в окне Services & prices. Правила подтверждены
владельцем и записаны в верхнем разделе хендоффа v5.1.

---

## Показатели строки и вид таблицы · 9 сентября 2026

В заказе появились Materials, Actual/Rounded/Billable Area, Unit/Line Weight и
Unit Price/Line Total. Materials показывает сырую стоимость материалов одного
юнита рядом с Services. Columns сохраняет видимость и произвольный порядок
всех колонок по профилю сотрудника в этом браузере. Pricing rules регулирует минимум
площади и надбавки Triple / Large unit; проценты складываются от исходной
цены готового юнита. Детализация открывается нажатием на показатель.

Пример площади: 47 × 73½″ → 23.9896 ft² фактически → 24.2 ft² после округления;
минимум к оплате по умолчанию 4 ft². Итог считается в порядке Subtotal → ES →
HST → Card fee → Delivery → Skid Deposit → Total. Delivery и Skid Deposit —
фиксированные суммы вне базы всех процентов. Вес собирается по всем компонентам;
отсутствующие нормы явно помечены и редактируются в детализации веса и во
вкладке Master Data → Weight norms. Сброс Shape очищает его геометрию и
сервисы, а все площади используют один текущий контур.
В итоговой строке валюта указана один раз рядом с Entire order. Активные суммы
идут подряд без пустот, Total закреплён справа, все значения имеют одинаковый
размер. Пользовательский интерфейс всегда
открывается на английском; переключатель RU / EN из шапки удалён.
Print находится в шапке заказа рядом с Close и Save/Update. Отдельной колонки
Status нет: ошибка подсвечивает Set красным и раскрывается нажатием. Нормы
spacer, desiccant и sealant показываются в g/ft², connectors — в g/unit.
Неизмеренные spacer-нормы выводятся из 30 g/ft² для 17/32 пропорционально
толщине и остаются расчётными до ручного переопределения;
масса Argon считается по объёму камеры.
Правила, ограничения и подтверждения владельца — в шапке handoff v5.4.
...
Проверенная локальная точка 24 сентября 2026: **837 passed, 0 failed** на
`src` и столько же на собранном `dist`, 85 браузерных скриптов. Это не
подтверждает безопасность файлов контроллеров или результат реального батча
2788 стёкол. Для проверки `TARGET=dist` сначала выполнить `npm run build`;
`npm run build:web` обновляет только `src/index.html`.
Историческая базовая точка 15 сентября (608 проверок после PR #80) описана в
архиве, но не должна использоваться как текущая.

Текущие решения и проверки: [`docs/GLASS_ERP_HANDOFF.md`](docs/GLASS_ERP_HANDOFF.md). История до 17 сентября — [`docs/HANDOFF_АРХИВ.md`](docs/HANDOFF_%D0%90%D0%A0%D0%A5%D0%98%D0%92.md).

---

## Что снаружи и не переписывается

- **Раскрой:** обязательная интеграция с Perfect Cut отменена 1 сентября 2026; собственный оптимизатор реализован. Perfect Cut используется как сравнительный образец, а не как движок ERP. Пробные адаптеры Maver/Disai выдают прямые файлы одного листа, но требуют проверки с оператором до реза; производственный экспорт ещё не готов — см. [`статус экспорта`](docs/MACHINE_EXPORT_STATUS.md).
