...
> Текущая точка 24 сентября 2026: очередь Optimization, батчи с Glass ID, NCR/Recut, стикеры и собственный раскрой с ручными правками. Раскрой учитывает безопасные сквозные резы, автоматический фактический левый Trim Y на отдельном листе, группы одинаковых схем и очередь размеров сырья. Кнопка Trial export выдаёт **отдельные файлы, не ZIP**, для одного открытого прямоугольного листа: Maver `.ISO`/`.BMP`, Disai `.dst`/`.sum`. Можно сохранить пару в выбранную папку, если браузер поддерживает эту возможность. Это **только для проверки без запуска реза**; производственный экспорт ещё не готов. 837 проверок для `src` и 837 для `dist`. Точное состояние и ограничения — в [`docs/GLASS_ERP_HANDOFF.md`](docs/GLASS_ERP_HANDOFF.md) и [`docs/MACHINE_EXPORT_STATUS.md`](docs/MACHINE_EXPORT_STATUS.md).
