...
> Текущая точка 24 сентября 2026: очередь Optimization, батчи с Glass ID, NCR/Recut, стикеры и собственный раскрой с ручными правками. Раскрой учитывает безопасные сквозные резы, автоматический фактический левый Trim Y на отдельном листе, группы одинаковых схем и очередь размеров сырья. Машинно-нейтральный снимок для Maver и Disai подготовлен, но **станочный экспорт ещё не готов**. 832 проверки для `src` и 832 для `dist`. Точное состояние и ограничения — в [`docs/GLASS_ERP_HANDOFF.md`](docs/GLASS_ERP_HANDOFF.md), следующий этап экспорта — в [`docs/MACHINE_EXPORT_STATUS.md`](docs/MACHINE_EXPORT_STATUS.md).
...
Проверенная локальная точка 24 сентября 2026: **832 passed, 0 failed** на
`src` и столько же на собранном `dist`, 84 браузерных скрипта. Это не
...
- **Раскрой:** обязательная интеграция с Perfect Cut отменена 1 сентября 2026; собственный оптимизатор реализован. Perfect Cut используется как сравнительный образец, а не как движок ERP. Экспорт программ Maver/Disai ещё требует отдельных адаптеров и проверки на столах — см. [`статус экспорта`](docs/MACHINE_EXPORT_STATUS.md).
