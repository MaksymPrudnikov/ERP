...
> **Уточнение 7 сентября 2026 (PR #54).** Это целевая архитектура, а не описание полностью реализованной системы. Поздние решения — в [хендоффе](GLASS_ERP_HANDOFF.md). Muntin уже хранится в `shape.muntin`, имеет общий чертёж с формой и не требует отдельной коллекции/ссылки. Решение 1 сентября о своём оптимизаторе после БД и реальных заказов заменяет исходный план обязательной интеграции с Perfect Cut.

Версия: 1.1 · 7 сентября 2026 (уточнены решения по Muntin и раскрою; исходный контур — 18 августа)
...
- По решению 1 сентября 2026 свой оптимизатор раскроя запланирован после БД и настоящих заказов. Упоминания адаптера Perfect Cut в целевых этапах ниже относятся к исходному варианту архитектуры; обязательство его реализовать отменено.
...
- Shape — единственный источник геометрии и владелец `shape.muntin`. Общее ядро Muntin получает определение Shape через адаптер; отдельной сохраняемой ссылки и второй ширины/высоты нет.
...
- IGU makeup: число lite, spacer, gas, sealants.
- Геометрия изделия и раскладка: Shape с `shape.muntin`, отдельно от Makeup/Cavity; уточнить производственные ограничения.
