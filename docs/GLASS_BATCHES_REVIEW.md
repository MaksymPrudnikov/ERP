...
- Glass ID — производственный номер: виден только в Optimization. В Sales
  (список, заказ, окна) номеров нет — решение владельца.
...
8. В Sales правой кнопкой на заказе с батчем → **Unbatch order…** (редкое
   действие, только для этого заказа). В окне батчи заказа; снятая галочка
   оставляет батч. Стёкла других заказов в тех же батчах не трогаются.
9. В Sales у частичного заказа: **Batched 🔒 · N/M pcs**. Ready, выдача и
...
28 проверок в `test/glass-batches.js`: Unbatch order из Sales, нет Glass ID в Sales, выдача номеров при сохранении, Verify, возврат в New, Cancel/Restore,
...
![Sales: Unbatch order в меню](glass-batches-screenshots/13-sales-unbatch-order-menu.png)
![Sales: окно Unbatch order](glass-batches-screenshots/14-sales-unbatch-order-dialog.png)
