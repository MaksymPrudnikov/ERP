...
- При **Verify** каждое стекло получает постоянный номер `G-0000001`
  (буква G и 7 цифр). Номер не меняется и не выдаётся повторно.
...
- Печать этикеток и сканирование — следующие этапы.

## Проверки

24 проверки в `test/glass-batches.js`: выдача номеров при Verify, сохранение,
