...
module.exports=async function({page,eq,ok}){
 ...
 eq('Service + opens editable charges and packs enabled totals without gaps',await t.p.evaluate(()=>{
  ...
  const print=document.createElement('div');print.innerHTML=salesOrderPrintMarkup();document.body.append(print);const printLayout=compactLayout(print.querySelector('.metric-order-total'));print.remove();
  ...
  return {before,printLayout,initial,after:[after.subtotal,after.energy,after.hst,after.card,after.delivery,after.skidDeposit,after.grand],shown:['ES','HST','Visa card fee','Delivery','Skid Deposit','Total'].every(x=>text.includes(x)),slots:document.querySelector('.metric-order-charge-preview').querySelectorAll('.metric-order-charge-item').length,printButton:/metric-order-service-add/.test(salesOrderPrintMarkup())};
 }),{before:{button:'Service +',layout:{labels:['Subtotal','ES 9.75%','HST 13%','Total'],contiguous:true,totalAnchored:true},currency:'CAD',currencyCount:1,amountCurrencyCount:0,amountFontSizes:['12px'],noLaptopOverflow:true},printLayout:{labels:['Subtotal','ES 9.75%','HST 13%','Total'],contiguous:true,totalAnchored:true},initial:{checked:2,rates:[9.75,13,2.34,0,0],cards:['visa','mastercard','amex']},after:[242,23.6,34.53,7.02,500,750,1557.15],shown:true,slots:7,printButton:false});
